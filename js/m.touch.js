...
function startMove(init) {
  ...
  init.time = init.time > 0 && init.time < 200
    ? 200
    : init.time;
  var t = 0;
  var b = {};
...
function mScroll(init) {
  ...
  IN.wrap.addEventListener('touchmove', function(e) {
    ...
    bar && (css(bar, IN.dir == "x"
      ? "left"
      : "top", -traslate[IN.dir] * scale));
  ...
  IN.wrap.addEventListener('touchend', function(e) {
    ...
    // 目标过界限制
    if (IN.over == "backOut" && (MaxT[IN.dir] < target || MinT[IN.dir] > target)) {
      MoveType = "backOut";
    } else {
  ...
  function getScroll() { //获取当前的滚动位置
    return {
      x: css(IN.scroll, "translateX"),
      y: css(IN.scroll, "translateY")
    };
  }
}
